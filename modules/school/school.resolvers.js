...
async function UpdateSchool(_, { id, updateSchoolInput }, context) {
  try {
    ...
    const updatedSchool = await SchoolModel.findOneAndUpdate({ _id: id }, { $set: updateSchoolPayload }, { new: true });
...
async function DeleteSchool(_, { id }, context) {
  try {
    ...
    if (students) {
      const deletedStudents = await StudentModel.updateMany(students.filter, students.update);
      if (!deletedStudents.nModified) {
        throw new ApolloError('No student match for deletion', 'STUDENT_NOT_FOUND');
      }
    }

    const deletedSchool = await SchoolModel.findOneAndUpdate(school.filter, school.update);
    if (!deletedSchool) {
