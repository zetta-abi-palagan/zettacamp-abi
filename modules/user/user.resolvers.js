...
async function UpdateUser(_, { id, updateUserInput }, context) {
  try {
    ...
    const updatedUser = await UserModel.findOneAndUpdate({ _id: id }, { $set: updateUserPayload }, { new: true }).lean();
...
async function DeleteUser(_, { id }, context) {
  try {
    ...
    const deletedUser = await UserModel.findOneAndUpdate(user.filter, user.update).lean();
