// *************** IMPORT LIBRARY ***************
const bcrypt = require('bcrypt');
const { ApolloError } = require('apollo-server');

// *************** IMPORT UTILITIES *************** 
const { SoftDelete } = require('../../shared/utils/database.utils');

// *************** IMPORT VALIDATOR ***************
const { IsValidObjectId } = require('../../shared/validator/object_id');
const { IsEmailValid, IsEmailUnique } = require('../../shared/validator/email');

// *************** QUERY ***************
/**
 * Fetches a list of all active users.
 * @param {object} context - The GraphQL context containing models.
 * @returns {Promise<Array<Object>>} A promise that resolves to an array of user objects.
 * If no users are found, returns an empty array.
 * @throws {ApolloError} If fetching users fails.
 */
async function GetAllUsers(_, _, context) {
    try {
        const users = await context.models.User.find({ deleted_at: null });
        return users;
    } catch (error) {
        throw new ApolloError(`Failed to fetch users: ${error.message}`, "INTERNAL_SERVER_ERROR");
    }
}

/**
 * Fetch a single user by ID, excluding soft-deleted users.
 * @param {any} _ - The unused parent/root argument from GraphQL resolvers.
 * @param {{ id: string }} args - An object containing the user's ID.
 * @param {string} args.id - The ID of the user to retrieve.
 * @param {object} context - The GraphQL context containing models.
 * @returns {Promise<Object>} A promise that resolves to the user object if found and not deleted.
 * @throws {ApolloError} If the ID is invalid, the user is not found, or a database error occurs.
 */
async function GetOneUser(_, { id }, context) {
    if (!IsValidObjectId(id)) {
        throw new ApolloError(`Invalid ID: ${id}`, "BAD_USER_INPUT");
    }

    try {
        const user = await context.models.User.findById({ _id: id, deleted_at: null });
        if (!user || user.deleted_at) {
            throw new ApolloError("User not found", "NOT_FOUND");
        }
        return user;
    } catch (error) {
        throw new ApolloError(`Failed to fetch user: ${error.message}`, "INTERNAL_SERVER_ERROR");
    }
}

// *************** MUTATION ***************
/**
 * Create a new user.
 * Validates the email format, ensures email uniqueness, hashes the password,
 * and stores the user data in the database.
 * @param {any} _ - The unused parent/root argument from GraphQL resolvers.
 * @param {{ input: { 
 *   first_name: string, 
 *   last_name: string, 
 *   email: string, 
 *   password: string, 
 *   role: string 
 * }}} args - The input object containing user data.
 * @param {object} context - The GraphQL context containing models.
 * @returns {Promise<void>} A promise that resolves when the user is successfully created.
 * @throws {ApolloError} If the email is invalid, not unique, required fields are missing,
 * or if any other error occurs during creation.
 */
async function CreateUser(_, { input }, context) {
    const { first_name, last_name, email, password, role } = input;

    if (!IsEmailValid(email)) {
        throw new ApolloError(`Invalid email format: ${email}`, "BAD_USER_INPUT");
    }

    try {
        const isEmailUnique = await IsEmailUnique(email, context.models.User);
        if (!isEmailUnique) {
            throw new ApolloError(`The email address '${email}' is already taken`, "BAD_USER_INPUT");
        }

        if (
            first_name === undefined ||
            last_name === undefined ||
            email === undefined ||
            password === undefined ||
            role === undefined
        ) {
            throw new ApolloError('All fields (first_name, last_name, email, password, role) are required.', "BAD_USER_INPUT");
        }

        const hashedPassword = await bcrypt.hash(password, 10);

        const user = {
            first_name,
            last_name,
            email,
            password: hashedPassword,
            role
        };

        return await context.models.User.create(user);
    } catch (error) {
        if (
            error.message.includes("is already taken") ||
            error.message.includes("Invalid email format")
        ) {
            throw new ApolloError(error.message, "BAD_USER_INPUT");
        }

        console.error(`CreateUser Resolver Error for email ${email}:`, error);
        throw new ApolloError(`Failed to create user: ${error.message}`, "INTERNAL_SERVER_ERROR");
    }
}

/**
 * Updates a user's information in the database.
 * Validates the provided ID and email format (if email is being updated).
 * Ensures email uniqueness if a new email is provided.
 * Hashes the password if a new password is provided.
 * Stores the updated user data.
 * @param {object} _ - The unused parent/root argument, typical in GraphQL resolvers.
 * * @param {{ id: string }} args - An object containing the user's ID.
 * @param {{ input: { 
 *   first_name: string, 
 *   last_name: string, 
 *   email: string, 
 *   password: string, 
 *   role: string 
 * }}} args - The input object containing user data.
 * @param {object} context - The GraphQL context containing models.
 * @returns {Promise<object>} A promise that resolves to the updated user object.
 * @throws {ApolloError} If the ID is invalid, email format is invalid (for new email),
 * email address is already taken (for new email), or if any other unexpected server error occurs.
 */
async function UpdateUser(_, { id, input }, context) {
    const { first_name, last_name, email, password, role } = input;

    if (!IsValidObjectId(id)) {
        throw new ApolloError(`Invalid ID: ${id}`, "BAD_USER_INPUT");
    }

    if (email && !IsEmailValid(email)) {
        throw new ApolloError(`Invalid email format: ${email}`, "BAD_USER_INPUT");
    }

    try {
        if (email) {
            const isEmailUnique = await IsEmailUnique(email, context.models.User);
            if (!isEmailUnique) {
                throw new ApolloError(`The email address '${email}' is already taken`, "BAD_USER_INPUT");
            }
        }

        const update = {};
        if (first_name !== undefined) update.first_name = first_name;
        if (last_name !== undefined) update.last_name = last_name;
        if (email !== undefined) update.email = email;
        if (role !== undefined) update.role = role;

        if (password) {
            const hashedPassword = await bcrypt.hash(password, 10);
            update.password = hashedPassword;
        }

        return await context.models.User.findByIdAndUpdate(id, update, { new: true });
    } catch (error) {
        if (
            error.message.includes("is already taken") ||
            error.message.includes("Invalid email format") ||
            error.message.includes("Invalid ID")
        ) {
            throw new ApolloError(error.message, "BAD_USER_INPUT");
        }

        console.error(`UpdateUser Resolver Error for ID ${id}:`, error);
        throw new ApolloError(`An unexpected server error occurred. Please try again later. ${error.message}`, "INTERNAL_SERVER_ERROR");
    }
}

/**
 * Finds a user by ID and soft deletes them using the `SoftDelete` utility
 * if the user exists and is not already soft-deleted.
 * @param {{ id: string }} args - An object containing the user's ID.
 * @param {object} context - The GraphQL context containing models.
 * @returns {Promise<object>} A promise that resolves to the soft-deleted user document.
 * @throws {ApolloError} Throws an error with message "User not found" if the user does not exist
 * or is already soft-deleted.
 */
async function DeleteUser(_, { id }, context) {
    if (!IsValidObjectId(id)) {
        throw new ApolloError(`Invalid ID: ${id}`, "BAD_USER_INPUT");
    }

    try {
        const user = await context.models.User.findById(id);
        if (!user || user.deleted_at) {
            throw new ApolloError('User not found', "NOT_FOUND");
        }

        return await SoftDelete(user);
    } catch (error) {
        throw new ApolloError(`Failed to delete user: ${error.message}`, "INTERNAL_SERVER_ERROR");
    }
}

// *************** EXPORT MODULE ***************
module.exports = {
    GetAllUsers,
    GetOneUser,
    CreateUser,
    UpdateUser,
    DeleteUser
}