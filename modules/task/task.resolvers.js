...
async function DeleteTask(_, { id }, context) {
  try {
    ...
    const { task, test } = await TaskHelper.GetDeleteTaskPayload({ taskId: id, userId });

    // *************** Soft-delete the task
    const deletedTask = await TaskModel.findOneAndUpdate(task.filter, task.update).lean();
...
async function AssignCorrector(_, { task_id, corrector_id, enter_marks_due_date }, context) {
  try {
    ...
    const completeAssignCorrectorTask = await TaskModel.updateOne(
      { _id: task_id, task_type: 'ASSIGN_CORRECTOR', task_status: 'PENDING' },
...
async function EnterMarks(_, { task_id, enterMarksInput, validate_marks_due_date }, context) {
  try {
    ...
    const taskCompletionPayload = TaskHelper.GetTaskCompletionPayload(userId);
    const completeEnterMarksTask = await TaskModel.updateOne(
      { _id: task_id, task_type: 'ENTER_MARKS', task_status: 'PENDING' },
      { $set: taskCompletionPayload }
    );
    ...
    const insertStudentTestResultToTest = await TestModel.updateOne(
      { _id: enterMarksInput.test },
      { $addToSet: { student_test_results: studentTestResult._id } }
    );
...
async function ValidateMarks(_, { task_id, student_test_result_id }, context) {
  try {
    ...
    const studentTestResultToValidate = await StudentTestResultModel.findOne({
    ...
    const validatedStudentTestResult = await StudentTestResultModel.findOneAndUpdate({ _id: student_test_result_id }, validationPayload, {
      new: true,
    }).lean();
