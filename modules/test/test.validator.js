  ...
  if (test_type) {
    ...
    if (isCompetencyMismatch || isScoreMismatch) {
      throw new ApolloError(
        `Test type '${upperTestType}' is not allowed for block with evaluation_type '${evaluationType}'.`,
        'BAD_USER_INPUT',
        { field: 'test_type' }
      );
    }
...
function validateSingleNotationCondition({ condition, availableNotationTexts, path }) {
  ...
  if (condition.criteria_type.toUpperCase() === 'MARK') {
    if (typeof condition.notation_text !== 'string' || condition.notation_text.trim() === '') {
      throw new ApolloError(
...
function ValidateStudentTestResultLoaderInput(test, context) {
  ...
  if (
    ...
    throw new ApolloError(
      'Server configuration error: StudentTestResultLoader with loadMany function not found on context.',
